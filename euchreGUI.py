...
class EuchreGUI:
    ...
    def __init__(self):
        ...
        for y in range(0, self.BUTTON_NUM_ROWS_1):
            for x in range(0, self.BUTTON_NUM_COLUMNS_1):
                self.bidButtonsRound1.append(box.Box(math.floor(self.width / 2 - (
                        self.BUTTON_WIDTH_ROW * self.BUTTON_NUM_COLUMNS_1) / 2 + self.BUTTON_WIDTH_ROW * x + self.BUTTON_OFFSET),
                                                     math.floor(self.height / 2 - (
                                                             self.BUTTON_HEIGHT_ROW * self.BUTTON_NUM_ROWS_1) / 2 + self.BUTTON_HEIGHT_ROW * y + self.BUTTON_OFFSET),
                                                     self.BUTTON_WIDTH_ROW - 2 * self.BUTTON_OFFSET,
                                                     self.BUTTON_HEIGHT_ROW - 2 * self.BUTTON_OFFSET))

        for y in range(0, self.BUTTON_NUM_ROWS_2):
            for x in range(0, self.BUTTON_NUM_COLUMNS_2):
                self.bidButtonsRound2.append(box.Box(math.floor(self.width / 2 - (
                        self.BUTTON_WIDTH_ROW * self.BUTTON_NUM_COLUMNS_2) / 2 + self.BUTTON_WIDTH_ROW * x + self.BUTTON_OFFSET),
                                                     math.floor(self.height / 2 - (
                                                             self.BUTTON_HEIGHT_ROW * self.BUTTON_NUM_ROWS_2) / 2 + self.BUTTON_HEIGHT_ROW * y + self.BUTTON_OFFSET),
                                                     self.BUTTON_WIDTH_ROW - 2 * self.BUTTON_OFFSET,
                                                     self.BUTTON_HEIGHT_ROW - 2 * self.BUTTON_OFFSET))
    ...
    def printBids(self):
        ...
        # printall:
        if (self.southBid == '' or self.southBid == 'p') and (self.westBid == '' or self.westBid == 'p') and (
                self.northBid == '' or self.northBid == 'p') and (self.eastBid == '' or self.eastBid == 'p'):
            printall = 1
        ...
        if self.southBid != '' and (printall == 1 or self.southBid != 'p'):
            ...
            self.screen.blit(label, (math.floor(self.screen_width / 2 - halfCharLength * len(self.southBid)),
                                     math.floor(self.screen_height - self.off_the_edgeY / 2 + 30)))

        if self.westBid != '' and (printall == 1 or self.westBid != 'p'):
            label = myFont.render(self.westBid, 1, (0, 0, 255))
            self.screen.blit(label, (
                20 + self.off_the_edgeX - halfCharLength * len(self.westBid), math.floor(self.screen_height / 2) - 30))

        if self.northBid != '' and (printall == 1 or self.northBid != 'p'):
            label = myFont.render(self.northBid, 1, (0, 0, 255))
            self.screen.blit(label,
                             (math.floor(self.screen_width / 2 - halfCharLength * len(self.northBid)), math.floor(self.off_the_edgeY / 4 - 30)))

        if self.eastBid != '' and (printall == 1 or self.eastBid != 'p'):
            label = myFont.render(self.eastBid, 1, (0, 0, 255))
            self.screen.blit(label, (
                self.screen_width - self.off_the_edgeX - halfCharLength * len(self.eastBid),
                math.floor(self.screen_height / 2 - 30)) )
    ...
    def printScore(self):
        ...
        pygame.draw.rect(self.screen, (255, 255, 255, 0), (math.floor((1 * self.width) / 32), math.floor((4 * self.height) / 5 + 10), 300, 200))
        # Make summation lines:
        pygame.draw.rect(self.screen, (0, 0, 255, 0),
                         (math.floor((1 * self.width) / 32), math.floor((4 * self.height) / 5 + 10 + 3 * 40), 60, 5))
        pygame.draw.rect(self.screen, (0, 0, 255, 0),
                         (math.floor((1 * self.width) / 32 + 70), math.floor((4 * self.height) / 5 + 10 + 3 * 40), 60, 5))
        ...
        with self.scoreLock:
            ...
            self.screen.blit(labelExample1, (math.floor((1 * self.width) / 32), math.floor((4 * self.height) / 5 + 10)))

            if self.diffScoreThem == FIRST_ROUND:
                # If it's the first round and don't put the previous scores up...
                pass
            else:
                self.screen.blit(labelExample2, (math.floor((1 * self.width) / 32), math.floor((4 * self.height) / 5 + 10 + 1 * 40)) )
                self.screen.blit(labelExample3, (math.floor((1 * self.width) / 32), math.floor((4 * self.height) / 5 + 10 + 2 * 40)) )
            self.screen.blit(labelExample4, (math.floor((1 * self.width) / 32), math.floor((4 * self.height) / 5 + 10 + 3 * 40)) )
    ...
    def printTricks(self):
        ...
        self.screen.blit(labelTricksWest, (5, math.floor(self.height / 2)))

        self.screen.blit(labelTricksEast, (1 * self.width - 85, math.floor(self.height / 2)))

        self.screen.blit(labelTricksNorth, (math.floor(self.width / 2), math.floor(self.height / 20)))

        self.screen.blit(labelTricksSouth, (math.floor(self.width / 2), self.height - 90))
    ...
    def shiftSouthCards(self, origIndex, isLeft, numSpaces):
        ...
        if len(self.southCards) > origIndex >= 0:
    ...
    def displayCenterGameMsg(self):
        ...
        self.screen.blit(labelExample, (math.floor(self.width / 2 - 10 * xOffset), math.floor(self.height / 2 - 50)) )
    ...
    def displayBidChoices(self):
        ...
        if self.biddingRound == 1:
            pygame.draw.rect(self.screen, (255, 0, 255, 0), (
                math.floor(self.width / 2 - self.BUTTON_WIDTH_ROW * self.BUTTON_NUM_COLUMNS_1 / 2),
                math.floor(self.height / 2 - self.BUTTON_HEIGHT_ROW * self.BUTTON_NUM_ROWS_1 / 2),
                self.BUTTON_WIDTH_ROW * self.BUTTON_NUM_COLUMNS_1,
                self.BUTTON_HEIGHT_ROW * self.BUTTON_NUM_ROWS_1))
            ...
            for i in range(0, len(self.bidButtonsRound1)):
                ...
                if self.variation == 'ontarian' and i == 1 and self.dealer.find('North(') != -1:
                    ...
                    # Draw an x:
                    pygame.draw.line(self.screen, (255, 0, 0, 0),
                                     (
                                         self.bidButtonsRound1[i].getX(),
                                         self.bidButtonsRound1[i].getY()
                                      ),
                                     (
                                         self.bidButtonsRound1[i].getX() + self.bidButtonsRound1[i].getWidth(),
                                         self.bidButtonsRound1[i].getY() + self.bidButtonsRound1[i].getHeight()
                                     ), 2)
                    pygame.draw.line(self.screen, (255, 0, 0, 0),
                                     (
                                         self.bidButtonsRound1[i].getX(),
                ...
                if i == 0:
                    ...
                    self.screen.blit(labelAlone,
                                     (self.bidButtonsRound1[i].getX(), self.bidButtonsRound1[i].getY() + nextLine))

        elif self.biddingRound == 2:

            pygame.draw.rect(self.screen, (255, 0, 255, 0), (
                self.width / 2 - self.BUTTON_WIDTH_ROW * self.BUTTON_NUM_COLUMNS_2 / 2,
                self.height / 2 - self.BUTTON_HEIGHT_ROW * self.BUTTON_NUM_ROWS_2 / 2,
                self.BUTTON_WIDTH_ROW * self.BUTTON_NUM_COLUMNS_2, self.BUTTON_HEIGHT_ROW * self.BUTTON_NUM_ROWS_2))
            ...
            for i in range(0, len(self.bidButtonsRound2)):
                ...
                if i != 5:
                    if (i % 5 == 1 and 'S' == initialTrump) or (i % 5 == 2 and 'H' == initialTrump) or (
                            i % 5 == 3 and 'C' == initialTrump) or (i % 5 == 4 and 'D' == initialTrump):
                        pygame.draw.rect(self.screen, (170, 183, 184, 0), self.bidButtonsRound2[i].getCoordBox())

                        # Draw an x:
                        pygame.draw.line(self.screen, (255, 0, 0, 0),
                                         (self.bidButtonsRound2[i].getX(), self.bidButtonsRound2[i].getY()), (
                                             self.bidButtonsRound2[i].getX() + self.bidButtonsRound2[i].getWidth(),
                                             self.bidButtonsRound2[i].getY() + self.bidButtonsRound2[i].getHeight()), 10)
                        pygame.draw.line(self.screen, (255, 0, 0, 0), (self.bidButtonsRound2[i].getX(),
                                                                       self.bidButtonsRound2[i].getY() +
                                                                       self.bidButtonsRound2[i].getHeight()), (
                                             self.bidButtonsRound2[i].getX() + self.bidButtonsRound2[i].getWidth(),
                                             self.bidButtonsRound2[i].getY()), 10)
                    else:
                ...
                if i >= 6:
                    nextLine = self.BUTTON_HEIGHT_ROW / 2 - self.BUTTON_OFFSET
                    self.screen.blit(labelAlone,
                                     (self.bidButtonsRound2[i].getX(), math.floor(self.bidButtonsRound2[i].getY() + nextLine)))
...
def main(connection):
    ...
    while 1:
        ...
        if euchreGUI.showBackToChannelButton == 1:
            returnToChannelPressed = returnToChannelButton.updateButtonAndCheckIfPressed(mx, my, mouseJustPressed,
                                                                                         mouseJustRelease)
        ...
        # TODO: Chat box:
        pygame.draw.rect(euchreGUI.screen, euchreGUI.WHITE,
                         [math.floor((6 * euchreGUI.width) / 8), math.floor((4 * euchreGUI.height) / 5 + 10), 300, 200])
        ...
        clock.tick(math.floor(1000 / euchreGUI.FRAME_WAIT_TIME))
