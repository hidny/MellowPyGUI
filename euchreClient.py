...
def isACard(card):
    if len(card) != 2:
        ...
        if card[0] == '2' or card[0] == '3' or card[0] == '4' or card[0] == '5' or card[0] == '6' or card[0] == '7' or \
                card[0] == '8' or card[0] == '9' or card[0] == 'T' or card[0] == 'J' or card[0] == 'Q' or card[
            0] == 'K' or card[0] == 'A':
        ...
        while euchreGUIVars.isGameOver() == 0:
            ...
            for currentLine in currentLines:
                ...
                if currentLine.startswith(PUBLIC_MSG):
                    ...
                    # if players[0]  == player:
                    # print('South(' + player + ') has ' + number + ' trick(s).')
                    # elif players[1] == player:
                    ...
                    if currentLine.find(PASSES) != -1 or currentLine.find(ORDER_UP) != -1 or currentLine.find(
                            PICK_UP) != -1 or currentLine.find(DECLARE) != -1:
                    ...
                    # if all other public messages are false, go here:
                        ...
                        if len(tokens) > 1 and isNumber(tokens[0]) == 1 and isNumber(
                                tokens[len(tokens) - 1]) == 1 and isRoundSetup == 0:
...
def main(euchreGUIVars, args):
    ...
    try:
        _thread.start_new_thread(serverListener, (connection, euchreGUIVars))
    except:
